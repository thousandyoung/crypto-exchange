...
import com.howellyoung.exchange.enums.BarTypeEnum;
...
/*
    process and manage Ticks, including update redis cache, save tick to db, update and save bars.
 */
@Component
public class QuotationService extends LoggerBase {
    ...
    public void processMessages(List<BaseMessage> messages) {
    ...
    void processMessage(TickMessage message) {
        ...
        // 将Redis返回的K线保存至数据库:
        Map<BarTypeEnum, BigDecimal[]> barMap = JsonUtil.readJson(strCreatedBars, TYPE_BARS);
        if (!barMap.isEmpty()) {
            SecBarEntity secBar = createBar(SecBarEntity::new, barMap.get(BarTypeEnum.SEC));
            MinBarEntity minBar = createBar(MinBarEntity::new, barMap.get(BarTypeEnum.MIN));
            HourBarEntity hourBar = createBar(HourBarEntity::new, barMap.get(BarTypeEnum.HOUR));
            DayBarEntity dayBar = createBar(DayBarEntity::new, barMap.get(BarTypeEnum.DAY));
            this.quotationDbService.saveBars(secBar, minBar, hourBar, dayBar);
        }
    }

    static <T extends BaseBarEntity> T createBar(Supplier<T> fn, BigDecimal[] data) {
        if (data == null) {
            return null;
        }
        T t = fn.get();
