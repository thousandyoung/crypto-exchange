...
import com.howellyoung.exchange.messaging.*;
...
/**
 * Sequence events. track event, prevent duplicate event, out of order event and missing event.
 */
@Component
public class SequenceService extends LoggerBase implements CommonErrorHandler {
    ...
    private void sendMessages(List<BaseEvent> messages) {
        this.messageProducer.sendMessages(messages);
    }

    private synchronized void processMessages(List<BaseEvent> messages) {
        if (!running || crash) {
            panic();
            return;
        }
